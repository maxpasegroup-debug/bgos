  ...
  return (
    <div className="group relative flex w-full justify-center">
      <Link
        href={href}
        title={label}
        className="relative flex w-full justify-center py-1"
      >
        <motion.span
          className={`relative flex h-11 w-11 items-center justify-center rounded-xl border transition-colors duration-300 sm:h-12 sm:w-12 ${
            active
              ? "border-[#FFC300]/40 bg-white/10 text-[#FFC300] shadow-[0_0_24px_rgba(255,195,0,0.2)]"
              : "border-transparent text-white/55 hover:border-white/10 hover:text-white"
          }`}
          whileHover={{
            scale: 1.04,
            boxShadow: active
              ? "0 0 28px rgba(255, 195, 0, 0.28)"
              : "0 0 20px rgba(255, 59, 59, 0.2), 0 0 36px rgba(255, 195, 0, 0.14)",
          }}
          whileTap={{ scale: 0.97 }}
      ...
      <span
        className="pointer-events-none absolute left-full top-1/2 z-50 ml-3 hidden -translate-y-1/2 whitespace-nowrap rounded-lg border border-white/10 bg-black/90 px-2.5 py-1.5 text-xs font-medium text-white opacity-0 shadow-lg backdrop-blur-md transition-opacity duration-200 group-hover:opacity-100 sm:block"
        role="tooltip"
      >
        {label}
      </span>
