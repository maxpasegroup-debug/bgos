...
export function OnboardingClientPage() {
  ...
  /** From `/api/auth/me` — “Select business type” only when no company yet. */
  const [meCompanyId, setMeCompanyId] = useState<string | null>(null);
  ...
  const probeSession = useCallback(async () => {
    ...
    const u = data.user;
    setMeCompanyId(typeof u.companyId === "string" && u.companyId.length > 0 ? u.companyId : null);

    const addBusinessIntent =
      (typeof window !== "undefined" && sessionStorage.getItem(BGOS_ADD_BUSINESS_INTENT_KEY) === "1") ||
      (typeof window !== "undefined" &&
        new URLSearchParams(window.location.search).get("addBusiness") === "1");
    if (u.role === "ADMIN" && u.companyId && !addBusinessIntent) {
    ...
    try {
      ...
      if (u?.needsWorkspaceActivation || u?.workspaceReady === false) {
        ...
        if (typeof data.companyId === "string" && data.companyId.length > 0) {
          await apiFetch("/api/company/switch", {
            method: "POST",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
  ...
  async function finishActivation(nextPath: string) {
    ...
    setPending(true);
    try {
      const res = await apiFetch("/api/onboarding/activate", {
        method: "POST",
        credentials: "include",
      });
          ...
          <div className="rounded-2xl border border-cyan-500/20 bg-[#0f141f]/90 p-8 shadow-[0_0_40px_-12px_rgba(34,211,238,0.35)] backdrop-blur-md">
            ...
            <div className="mt-8 flex flex-col gap-3">
              ...
              <button
                type="button"
                disabled={pending}
                onClick={() => void finishActivation("/bgos/dashboard")}
  ...
  if (step === 1 && !addBusinessFlow && !pickedSolarPath && meCompanyId === null) {
    return (
