...
export function emptyFinancialOverview(now = new Date()): DashboardFinancialOverview {
  return {
    ...
    monthlyExpenseTrend: buildDefaultFinancialTrend(now),
...
export function normalizeFinancialOverview(raw: unknown): DashboardFinancialOverview {
  ...
  const expTrendRaw = f.monthlyExpenseTrend;
  let monthlyExpenseTrend = base.monthlyExpenseTrend;
  if (Array.isArray(expTrendRaw) && expTrendRaw.length > 0) {
    monthlyExpenseTrend = expTrendRaw.map((p) => {
      if (!p || typeof p !== "object") {
        return { monthKey: "", label: "", amount: 0 };
      }
      const o = p as Record<string, unknown>;
  ...
  return {
    ...
    unpaidInvoiceCount:
      typeof f.unpaidInvoiceCount === "number" && Number.isFinite(f.unpaidInvoiceCount)
    ...
    currentMonthExpenses:
      typeof f.currentMonthExpenses === "number" && Number.isFinite(f.currentMonthExpenses)
        ? f.currentMonthExpenses
        : typeof f.currentMonthTotal === "number" && Number.isFinite(f.currentMonthTotal)
          ? f.currentMonthTotal
          : 0,
